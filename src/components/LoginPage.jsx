...
export default function LoginPage({ onLogin }) {
  ...
  const handleStudentRegister = async (e) => {
    ...
    if (!studentId.trim())   { setError('Student ID is required (e.g. A001).'); return; }
    if (!username.trim())    { setError('Username is required.'); return; }
    if (username.length < 3) { setError('Username must be at least 3 characters.'); return; }
    if (!password)           { setError('Password is required.'); return; }
    if (password.length < 4) { setError('Password must be at least 4 characters.'); return; }
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{
            ...
            boxShadow: '0 4px 16px rgba(74,85,104,0.2)',
          }}>🪑</div>
          <h1 style={{
        ...
        <div className="card" style={{ padding: '2rem' }}>
            ...
            {[
              ...
              <button key={m.id} type="button"
                ...
                style={{
                  ...
                  boxShadow: mode === m.id
                    ? '0 2px 8px rgba(74,85,104,0.2)' : 'none',
                }}>
                {m.label}
              </button>
            ))}
            ...
            <form onSubmit={handleLogin}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <button type="submit" className="btn btn-primary"
                  disabled={loading}
                  style={{ width: '100%', padding: 11, fontSize: '0.88rem' }}>
                  {loading
          ...
          {mode === 'student-register' && (
            <form onSubmit={handleStudentRegister}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div className="banner banner-info" style={{ margin: 0 }}>
                  ℹ Students can view their seat after admin generates seating.
                </div>
                <div className="form-group">
